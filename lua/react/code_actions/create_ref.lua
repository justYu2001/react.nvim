...
-- HTML tag → DOM interface map
local HTML_ELEMENT_TYPES = {
    a = "HTMLAnchorElement",
    abbr = "HTMLElement",
    address = "HTMLElement",
...
-- _version_override: optional 3rd arg to skip filesystem lookup (for tests)
function M.get_ref_type(component_name, is_typescript, _version_override)
    if not is_typescript then
        return nil, nil
    end

    if is_pascal_case(component_name) then
        local version = _version_override or M.get_react_version()
        ...
        -- HTML element
        local dom_type = HTML_ELEMENT_TYPES[component_name] or "HTMLElement"
        return dom_type, nil
    end
end
                ...
                return {
                    {
                        ...
                        action = function()
                            ...
                            local is_typescript =
                                filetype == "typescriptreact" or filetype == "typescript"
                            ...
                            local ref_type, extra_import =
                                M.get_ref_type(component_name, is_typescript)
                            ...
                            vim.schedule(function()
                                local inserted_line =
                                    vim.api.nvim_buf_get_lines(bufnr, decl_row, decl_row + 1, false)[
                                        1
                                    ]
...
-- Exports for testing
M.HTML_ELEMENT_TYPES = HTML_ELEMENT_TYPES
