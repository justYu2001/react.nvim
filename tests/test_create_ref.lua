...
T["get_ref_type"]["HTML element in TS → concrete DOM type, no extra import"] = function()
    local type_str, extra = create_ref.get_ref_type("div", true)
    eq(type_str, "HTMLDivElement")
    eq(extra, nil)
end

T["get_ref_type"]["HTML button in TS"] = function()
    local type_str, extra = create_ref.get_ref_type("button", true)
    eq(type_str, "HTMLButtonElement")
    eq(extra, nil)
end

T["get_ref_type"]["HTML input in TS"] = function()
    local type_str, extra = create_ref.get_ref_type("input", true)
    eq(type_str, "HTMLInputElement")
    eq(extra, nil)
end

T["get_ref_type"]["unknown lowercase tag falls back to HTMLElement"] = function()
    local type_str, extra = create_ref.get_ref_type("mywidget", true)
    eq(type_str, "HTMLElement")
    eq(extra, nil)
end

T["get_ref_type"]["custom component React < 19 → ElementRef"] = function()
    local type_str, extra = create_ref.get_ref_type("MyComp", true, 18)
    eq(type_str, "ElementRef<typeof MyComp>")
    eq(extra, "ElementRef")
end

T["get_ref_type"]["custom component React >= 19 → ComponentRef"] = function()
    local type_str, extra = create_ref.get_ref_type("MyComp", true, 19)
...
T["get_ref_type"]["custom component React 20 → ComponentRef"] = function()
    local type_str, extra = create_ref.get_ref_type("MyComp", true, 20)
    eq(type_str, "ComponentRef<typeof MyComp>")
    eq(extra, "ComponentRef")
end

T["get_ref_type"]["custom component version nil (unknown) → ElementRef fallback"] = function()
    local type_str, extra = create_ref.get_ref_type("MyComp", true, nil)
    eq(type_str, "ElementRef<typeof MyComp>")
    eq(extra, "ElementRef")
end

T["get_ref_type"]["JS any element → no type, no import"] = function()
    local type_str, extra = create_ref.get_ref_type("div", false)
    eq(type_str, nil)
    eq(extra, nil)
end

T["get_ref_type"]["JS custom component → no type, no import"] = function()
    local type_str, extra = create_ref.get_ref_type("MyComp", false)
    eq(type_str, nil)
    eq(extra, nil)
end
